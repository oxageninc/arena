...
For headline resolve-rate claims, run this same protocol over [SWE-bench Verified](https://github.com/SWE-bench/SWE-bench) with a containerized runner such as [Harbor](https://github.com/laude-institute/harbor) and the **official** evaluator, with a pre-registered instance list (published random seed over the 500 verified instances, n ≥ 100). Arena's local suite is for harness development, smoke comparisons, and metric plumbing — the statistics and receipt discipline are identical either way.
